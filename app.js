...
CF.handleLogin = function(e) {
  ...
  setTimeout(() => CF.navigate('dashboard'), 500);
};
...
CF.handleRegister = function(e) {
  ...
  CF.state.user = {
    ...
    role, institution: document.getElementById('regInst')?.value || '',
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-count]').forEach(el => {
    ...
    const step = ts => {
      ...
      el.textContent = (Number.isInteger(target) ? Math.round(eased*target).toLocaleString() : (eased*target).toFixed(1)) + suffix;
