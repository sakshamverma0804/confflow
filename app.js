...
// Build a CF user object from a Firebase user + Firestore profile
...
CF.handleLogin = function(e) {
  ...
  if (!window.__FIREBASE_READY__) {
    CF.toast('Firebase not configured — check build logs', 'error'); return;
  }
  ...
  window.FirebaseServices.AuthService.login(email, password)
    .then(async (fbUser) => {
      let profile = null;
      try { profile = await window.FirebaseServices.UserService.getProfile(fbUser.uid); } catch (_) {}
      CF.state.user = CF._buildUser(fbUser, profile);
    ...
    .catch((err) => {
      ...
      CF.toast(msgs[err.code] || 'Sign in failed. Please try again.', 'error');
    });
};
...
CF.handleRegister = function(e) {
  ...
  window.FirebaseServices.AuthService.register(email, password, displayName, role, inst)
    .then((fbUser) => {
      CF.state.user = CF._buildUser(fbUser, { displayName, role, institution: inst });
      CF.state.role = role;
    ...
    .catch((err) => {
      ...
      CF.toast(msgs[err.code] || 'Registration failed. Please try again.', 'error');
    });
};
...
CF.renderConferences = function() {
  ...
  if (!confs.length) {
    grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1">
      <div class="empty-icon">🔍</div><h4>No conferences found</h4>
      <p>Try different search terms or filters</p></div>`;
    return;
  }
  grid.innerHTML = confs.map(c => `
    <article class="conf-card" onclick="CF.openConf(${c.id})" style="--card-accent:${c.color}">
      ...
      <div class="conf-card__body">
        ...
        <div class="conf-card__meta"><span>${c.flag} ${c.location}</span><span>📅 ${c.date}</span></div>
      </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Restores session on page reload and reacts to sign-out in other tabs.
  function attachAuthListener() {
    ...
    window.FirebaseServices.AuthService.onAuthChange(async (fbUser) => {
      if (fbUser && !CF.state.user) {
        // Restore session after page reload
        let profile = null;
        try { profile = await window.FirebaseServices.UserService.getProfile(fbUser.uid); } catch (_) {}
        CF.state.user = CF._buildUser(fbUser, profile);
        CF.state.role = CF.state.user.role;
      } else if (!fbUser && CF.state.user) {
  ...
  // firebase.js fires 'firebase-ready' when init is done
  window.addEventListener('firebase-ready', (e) => {
    if (e.detail && e.detail.ok) attachAuthListener();
    else console.warn('Firebase not ready — running without real auth. Check env vars in Vercel.');
  }, { once: true });
});
